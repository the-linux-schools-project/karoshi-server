	...
	if ($ldapconnected == 1){
		...
		if(isset($_POST['queryString'])) {
			...
			if(strlen($queryString) > 1) {
				...
				if($ldapconnected == 1){
	         					$attributes= array("gidNumber","cn","uid");
                                                        $lookupString = "cn=$queryString*";                               
	         					if($userresults=@ldap_search($ldapconnection, "ou=People, dc=karoshi, dc=local",$lookupString, $attributes)){
                                                           ...
                                                           if($info["count"] > 0){
                                                            ...
                                                            while($entry) {
                                                                ...
                                                                if(strlen($name) > 35) {
                                                                    ...
                                                                    $userName = (@ldap_get_values($ldapconnection,$entry,'uid'));
                                                                    $groupDisplayName = array("Unknown");                              
                                                                                $gidNumber = (@ldap_get_values($ldapconnection,$entry,'gidnumber'));
                                                                                $attributes= array("displayName");
                                                                                $gidNumber = $gidNumber[0];
                                                                                $primarygroupnameresults=@ldap_search($ldapconnection, "ou=Group, dc=karoshi,dc=local", "gidNumber=$gidNumber", $attributes);
                                                                                if($entry2=@ldap_first_entry($ldapconnection,$primarygroupnameresults)){
                                                                                    $groupDisplayName = (@ldap_get_values($ldapconnection,$entry2,'displayname'));
                                                                                }
