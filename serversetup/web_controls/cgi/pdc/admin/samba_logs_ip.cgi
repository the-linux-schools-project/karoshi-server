...
echo ""
echo '<!DOCTYPE html><html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8"><title>'$"TCPIP Logs"'</title><link rel="stylesheet" href="/css/'$STYLESHEET'?d='$VERSION'"><script src="/all/stuHover.js" type="text/javascript"></script></head><body><div id="pagecontainer">'
...
echo 'alert("'$MESSAGE'")';
echo '                window.location = "/cgi-bin/admin/samba_logs_ip_fm.cgi";'
echo '</script>'
echo "</div></body></html>"
exit
}
...
Checksum=`sha256sum /var/www/cgi-bin_karoshi/admin/samba_logs_ip.cgi | cut -d' ' -f1`
#View logs
echo "$REMOTE_USER:$REMOTE_ADDR:$Checksum:$TCPIP:$DAY:$MONTH:$YEAR:" | sudo -H /opt/karoshi/web_controls/exec/samba_logs_ip
EXEC_STATUS=`echo $?`
if [ $EXEC_STATUS = 101 ]
then
