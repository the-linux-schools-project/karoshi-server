...
echo ""
echo '<!DOCTYPE html><html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8"><title>'$"Network Share Logs"'</title><link rel="stylesheet" href="/css/'$STYLESHEET'?d='$VERSION'"><script src="/all/stuHover.js" type="text/javascript"></script></head><body><div id="pagecontainer">'
...
if [ $SHARE'null' = null ]
then
MESSAGE=$"The share name must not be blank."
show_status
fi
...
Checksum=`sha256sum /var/www/cgi-bin_karoshi/admin/samba_logs_share.cgi | cut -d' ' -f1`
#View logs
echo "$REMOTE_USER:$REMOTE_ADDR:$Checksum:$SHARE:$DAY:$MONTH:$YEAR:" | sudo -H /opt/karoshi/web_controls/exec/samba_logs_share
EXEC_STATUS=`echo $?`
if [ $EXEC_STATUS = 101 ]
then
...
if [ $EXEC_STATUS = 102 ]
then
echo '<b>'$"Network Share Logs" $SHARE $DAY-$MONTH-$YEAR : $"No log for date."'</b><br><br>'
fi
