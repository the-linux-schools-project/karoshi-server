...
echo ""
echo '<!DOCTYPE html><html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8"><title>'$"Default User Settings"'</title><meta http-equiv="REFRESH" content="0; URL='$HTTP_REFERER'"><link rel="stylesheet" href="/css/'$STYLESHEET'?d='`date +%F`'"></head><body><div id="pagecontainer">'
#########################
#Get data input
#########################
...
#Modify settings
echo "$REMOTE_USER:$REMOTE_ADDR:$MD5SUM:SETDATA:$LOCKOUTDURATION:$LOCKOUTTHRESHOLD:$LOCKOUTOBS:$SHADOWMAX:$USERNAMESTYLE:$STUDENTMINPASSWORDLENGTH:$STUDENTCHARSANDNUMBERS:$STUDENTUPPERANDLOWERCASE:$STAFFMINPASSWORDLENGTH:$STAFFCHARSANDNUMBERS:$STAFFUPPERANDLOWERCASE:$CHANGEPASSFIRSTLOGIN:" | sudo -H /opt/karoshi/web_controls/exec/default_user_settings
EXEC_STATUS=`echo $?`
if [ $EXEC_STATUS != 0 ]
then
