...
echo ""
echo '<!DOCTYPE html><html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8"><title>'$"Create Commercial Certificate"'</title><link rel="stylesheet" href="/css/'$STYLESHEET'?d='$VERSION'"></head><body><div id="pagecontainer">'
#########################
#Get data input
#########################
...
then
	...
	echo "$REMOTE_USER:$REMOTE_ADDR:$Checksum:$REALM:checkservercsr:" | sudo -H /opt/karoshi/web_controls/exec/ssl_commercial_certs
	if [ $? = 105 ]
	then
...
then
	...
	echo "$REMOTE_USER:$REMOTE_ADDR:$Checksum:$SERVERNAME:$ACTION:" | sudo -H /opt/karoshi/web_controls/exec/ssl_commercial_certs
...
if [ $ACTION = installcertinfo ]
then
	echo "$REMOTE_USER:$REMOTE_ADDR:$Checksum:$SERVERNAME:$ACTION::::::::$CERTTYPE:$CACERT:$SSLCERT:" | sudo -H /opt/karoshi/web_controls/exec/ssl_commercial_certs
	MESSAGE=$"The SSL Certificate has been installed."
	show_status
	exit
fi

echo "$REMOTE_USER:$REMOTE_ADDR:$Checksum:$SERVERNAME:$ACTION:$COUNTRYCODE:$STATE:$LOCALITY:$INSTITUTENAME:$DEPARTMENT:$COMMONNAME:$EMAIL:" | sudo -H /opt/karoshi/web_controls/exec/ssl_commercial_certs
