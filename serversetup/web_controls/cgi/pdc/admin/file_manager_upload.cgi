...
$SERVERMASTER = param('SERVERMASTER');
...
#
system "[ -d /var/www/karoshi/file_manager_upload/$ENV{REMOTE_USER} ] && rm -f -R /var/www/karoshi/file_manager_upload/$ENV{REMOTE_USER}";
system "mkdir -p /var/www/karoshi/file_manager_upload/$ENV{REMOTE_USER}";
system "chmod 0700 /var/www/karoshi/";
system "chmod 0700 /var/www/karoshi/file_manager_upload/$ENV{REMOTE_USER}";
BEGIN {

	$SAVE_DIRECTORY = "/var/www/karoshi/file_manager_upload/$ENV{REMOTE_USER}";
                              #
                              # --> Variable:
                              #         $SAVE_DIRECTORY
                              # --> Function:
...
__END_OF_HTML_CODE__
	...
	if ($SUCCESS_LOCATION !~ /^\s*$/) {
		...
		<body>
		<form METHOD=POST ACTION="file_manager.cgi" name="frm">
			<input type="hidden" name="_DoUpload_" value="ACTION_REALLYUPLOADFILE_SERVERNAME_$SERVERNAME\_SERVERTYPE_$SERVERTYPE\_SERVERMASTER_$SERVERMASTER\_LOCATION_$LOCATION\_">
		</form>
		<script>
			document.frm.submit();
		</script>
