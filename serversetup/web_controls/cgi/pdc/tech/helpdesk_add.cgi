...
<link rel="stylesheet" href="/css/'$STYLESHEET'?d='$VERSION'"><meta http-equiv="REFRESH" content="0;url=helpdesk_view_fm.cgi"><title>'$TITLE'</title></head><body><div id="pagecontainer">'
#########################
#Get data input
#########################
...
echo 'alert("'$MESSAGE'")';
echo 'window.location = "/cgi-bin/tech/helpdesk_add_fm.cgi";'
echo '</script>'
echo "</div></body></html>"
exit
}
...
JOBTITLE=`echo $JOBTITLE | sed 's/%0D%0A/ /g' | sed 's/+/ /g' | sed 's/%3E//g' | sed 's/%27//g' | sed 's/%22//g' | sed 's/%3F/?/g' | sed 's/%2C/,/g' | sed 's/%21/!/g' | sed 's/%2F/\//g'`
REQUEST=`echo $REQUEST | sed 's/%0D%0A/ /g' | sed 's/+/ /g' | sed 's/%3E//g' | sed 's/%27//g'  | sed 's/%22//g' | sed 's/%3F/?/g' | sed 's/%2C/,/g' | sed 's/%21/!/g' | sed 's/%2F/\//g' | sed 's/%28/(/g' | sed 's/%29/)/g'`
DEPARTMENT=`echo $DEPARTMENT | sed 's/%24/$/g'`
